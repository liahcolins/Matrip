...
function populatePasseiosSelect(data) {
  filterPasseio.innerHTML = `<option value="">Todos</option>`;
...
async function loadPasseiosDoGuia() {
  const response = await fetch(`${API_BASE_URL}/guias/${currentUser.id}/passeios`);
  if (!response.ok) {
...
async function loadServices() {
  ...
  try {
    ...
    allServices = all.filter(item => String(item.guia_id) === String(currentUser.id));
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  sidebarGuideEmail.textContent = currentUser.email || "--";

  try {
    await loadPasseiosDoGuia();
    await loadServices();
  } catch (error) {
    showMessage("error", error.message);
  }
